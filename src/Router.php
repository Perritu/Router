...
class Router
{
     ...
     *
     * @param string Criteria to be used.
    ...
    public static function Evaluate(string $Criteria, int $Flags)
    {
        ...
        if (self::E_FLAT & $Flags) { // Flat plain-text evaluation mode.
            if (self::E_CASE_INSENSITIVE & $Flags)
            return strtolower($Reference) == strtolower(self::$RequestPath);

            return $Reference == self::$RequestPath;
        }

        if (self::E_PREG & $Flags) { // Regular expression evaluation mode.
            if (self::E_CASE_INSENSITIVE & $Flags)
    ...
    public static function MATCH(
        ...
        if (is_null(self::$RequestPath)) new \Perritu\Router\Router();
     ...
     */
    static private function PerformCall(
        callable | string $Callback,
        array $Params
    )
    {
